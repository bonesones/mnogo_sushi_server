...
class User {
    async signup(req, res, next) {
        ...
        try {
            ...
            res.status(202).cookie('token', token, { httpOnly: true, sameSite: 'none', secure: true , expires: new Date(Date.now() + (1000 * 60 * 60 * 24)) }).send('Куки установлены')
        } catch(e) {
    ...
    async login(req, res, next) {
        ...
        try {
            ...
            const token = createJWT(user.id, user.email, user.role)
            res.status(202).cookie('token', token, { httpOnly: true, secure: true, sameSite: "none", expires: new Date(Date.now() + (1000 * 60 * 60 * 24)) }).send('Куки установлены')
        } catch(e) {
            next(AppError.badRequest(e.message))
        }
    ...
    async logout(req, res, next) {
        try {
            ...
            res.status(202).cookie('token', "deleted", { httpOnly: true, secure: true, sameSite: "noneSS", expires: new Date(Date.now() - 1)}).send('Куки установлены')
        } catch(e) {
            next(AppError.badRequest(e.message))
        }
    }
